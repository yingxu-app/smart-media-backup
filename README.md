...
1. 下载 `YINGXU-macOS-v1.0.31.dmg`，双击打开。
...
- 20 项自动回归测试覆盖空卡、取消、目标不可用、空间不足、权限不足、同内容不同文件、重复跳过、拔卡模拟、多目标部分失败、数据库迁移、校验发现损坏文件、AI 不可用时扫描仍即时返回、导航页不会在后台反复扫描原卡，以及并发请求不会重复遍历同一张外置卡。

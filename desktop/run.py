"""影序 macOS 本地窗口入口：启动本地服务并嵌入 WebKit，而不是打开浏览器。"""
import os
import sys
import threading
import time
...
def main():
    ...
    html,body{margin:0;width:100%;height:100%;background:#0a0a0b;color:#e2e8f0;font-family:-apple-system,BlinkMacSystemFont,sans-serif}
    ...
    def load_workbench():
        if _wait_until_ready(url):
            ...
            window.load_html("<body style='background:#0a0a0b;color:#fecaca;font-family:-apple-system;padding:48px'><h2>影序启动失败</h2><p>本地服务未能在 15 秒内启动，请退出后重试。</p></body>")

    webview.start(load_workbench, gui="cocoa")
